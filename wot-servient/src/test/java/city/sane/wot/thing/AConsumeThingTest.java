...
/**
 * Abstract class to test Protocol Bindings
 * 
 * @author wolfposd
 */
public abstract class AConsumeThingTest {
    private Servient servient;

    /**
     * Provides the ProtocolServer and the ProtocolClientFactory classes to be used in the Binding test
     */
    public abstract Pair<Class<? extends ProtocolServer>, Class<? extends ProtocolClientFactory>> getServientClass();

    @Before
    public void setup() {
        Config config = ConfigFactory
                .parseString("wot.servient.servers = [\"" + getServientClass().first().getName() + "\"]\n"
                        + "wot.servient.client-factories = [\"" + getServientClass().second().getName() + "\"]")
    ...
    public void observeProperty() throws ConsumedThingException, ExecutionException, InterruptedException {
        ...
        Subscription subscription1 = consumedThing.getProperty("count").subscribe(next -> counter1.getAndIncrement()).get();

        AtomicInteger counter2 = new AtomicInteger();
        Subscription subscription2 = consumedThing.getProperty("count").subscribe(next -> counter2.getAndIncrement()).get();

        // wait until client establish subcription
        // TODO: This is error-prone. We need a feature that notifies us when the
        ...
        // Subscriptions are executed asynchronously. Therefore, wait "some" time before
        // we check the result.
        // TODO: This is error-prone. We need a function that notifies us when all
        // subscriptions have been executed.
        Thread.sleep(5 * 1000L);
    ...
    public void emitEvent() throws ConsumedThingException, InterruptedException, ExecutionException {
        ...
        AtomicInteger counter1 = new AtomicInteger();
        Subscription subscription1 = consumedThing.getEvent("change").subscribe(next -> counter1.getAndIncrement()).get();

        AtomicInteger counter2 = new AtomicInteger();
        Subscription subscription2 = consumedThing.getEvent("change").subscribe(next -> counter2.getAndIncrement()).get();
    ...
    private ExposedThing getExposedCounterThing() {
        ThingProperty counterProperty = new ThingProperty.Builder().setType("integer").setDescription("current counter content")
                .setObservable(true).build();

        ThingProperty lastChangeProperty = new ThingProperty.Builder().setType("string").setDescription("last change of counter content")
                .setObservable(true).setReadOnly(true).build();

        ExposedThing thing = new ExposedThing(servient).setId("counter").setTitle("counter");

        thing.addProperty("count", counterProperty, 42);
        thing.addProperty("lastChange", lastChangeProperty, new Date().toString());

        thing.addAction("increment", new ThingAction.Builder()
                .setDescription("Incrementing counter content with optional step content as uriVariable").setUriVariables(new HashMap<>() {
                    {
                ...
                }).setInput(new ObjectSchema()).setOutput(new IntegerSchema()).build(), (input, options) -> {
        ...
        thing.addAction("uppercase", new ThingAction.Builder()
                .setDescription("Converts all of the characters in this {@code String} to upper case.").setUriVariables(new HashMap<>() {
                    {
                ...
                }).setInput(new ObjectSchema()).setOutput(new StringSchema()).build(), (input, options) -> {
